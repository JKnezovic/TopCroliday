...
const ReservationTile = ({name,startDate, endDate, accommodation, choices, username, password}) => {
  ...
  return (
      <Pressable onPress={()=>setModalVisible(!modalVisible)} onLongPress={()=>onShare()} style={styles.container} >
        <View style={{width:"100%"}}>
            <Text style={styles.title}>{name}</Text>
            <View style={{flexDirection:'row', justifyContent:'space-around',width:"100%", paddingBottom:20 }}>
              <Text style={{fontWeight:'bold',color:'#092240'}}>{accommodation}</Text>
            ...
            {modalVisible&&
            <View>
             <ChoicesTile color={'#007d8c'} name={'Activities'} data={choices?.activities}></ChoicesTile>
             <ChoicesTile color={'#00ac8d'} name={'Transfer'} data={choices?.transfer}></ChoicesTile>
             <ChoicesTile color={'#004e70'} name={'Food & Drinks'} data={choices?.fridgeRestock}></ChoicesTile>
             <ChoicesTile color={'#007d8c'} name={'Cleaning'} data={choices?.cleaningServices}></ChoicesTile>
            </View>
